...
class CellGauge(ttk.Frame):
    ...
    def __init__(self, master: tk.Misc, title: str):
        ...
        self._photo: Optional["PhotoImage"] = None
    ...
    @property
    def gauge_number(self) -> str:
        digits = "".join(ch for ch in self.title if ch.isdigit())
        return digits or self.title
    ...
    def _scaled_point(self, angle_deg: float, radius: float) -> tuple[float, float]:
        x, y = self._point_from_angle(angle_deg, radius)
        return (x * self.AA_SCALE, y * self.AA_SCALE)

    def _scaled_box(self) -> tuple[float, float, float, float]:
        return (
            (self.center_x - self.radius) * self.AA_SCALE,
            (self.center_y - self.radius) * self.AA_SCALE,
            (self.center_x + self.radius) * self.AA_SCALE,
            (self.center_y + self.radius) * self.AA_SCALE,
        )

    def _render_with_pillow(self, value_v: Optional[float]) -> None:
        bg = "#f7f7f5"
        img = Image.new("RGBA", (self.width * self.AA_SCALE, self.height * self.AA_SCALE), bg)
        draw = ImageDraw.Draw(img)
        arc_box = self._scaled_box()
        ...
        for start_v, end_v, color in [
            ...
            draw.arc(
                ...
                fill=color,
                width=9 * self.AA_SCALE,
            )

        draw.arc(
            arc_box,
            start=-(150 + self.ROTATION_DEG),
            end=-(-90 + self.ROTATION_DEG),
            fill="#4a4a4a",
            width=2 * self.AA_SCALE,
        )

        for voltage in (0, 1, 2, 3, 4, 5):
            angle = self._angle_from_value(voltage)
            inner = self._scaled_point(angle, self.radius - 10)
            outer = self._scaled_point(angle, self.radius + 3)
            label = self._scaled_point(angle, self.radius + 15)
            draw.line([inner, outer], fill="#6f7782", width=2 * self.AA_SCALE)
            draw.text((label[0] - 8 * self.AA_SCALE, label[1] - 7 * self.AA_SCALE), str(voltage), fill="#6f7782", font=small_font)

        cx = self.center_x * self.AA_SCALE
        cy = self.center_y * self.AA_SCALE
        draw.ellipse((cx - 5 * self.AA_SCALE, cy - 5 * self.AA_SCALE, cx + 5 * self.AA_SCALE, cy + 5 * self.AA_SCALE), fill="#303030")
        badge_left = (self.center_x - 14) * self.AA_SCALE
        badge_top = (self.center_y - self.radius - 6) * self.AA_SCALE
        badge_right = (self.center_x + 14) * self.AA_SCALE
        badge_bottom = (self.center_y - self.radius + 22) * self.AA_SCALE
        draw.rounded_rectangle(
            (badge_left, badge_top, badge_right, badge_bottom),
            radius=6 * self.AA_SCALE,
            fill="#ffffff",
            outline="#4a4a4a",
            width=2 * self.AA_SCALE,
        )
        draw.text(
            ((self.center_x - 4) * self.AA_SCALE, (self.center_y - self.radius - 1) * self.AA_SCALE),
            self.gauge_number,
            fill="#202020",
            font=value_font,
        )
        ...
        if value_v is None or value_v <= 0:
            draw.text(
                (cx - 11 * self.AA_SCALE, (self.center_y - 42) * self.AA_SCALE),
                "OFF",
                fill="#6b7280",
                font=value_font,
            )
            off_point = (
                (self.center_x - self.radius + 18) * self.AA_SCALE,
                (self.center_y - 14) * self.AA_SCALE,
            )
            draw.line([(cx, cy), off_point], fill="#b0b6bf", width=4 * self.AA_SCALE)
        else:
            angle = self._angle_from_value(value_v)
            needle = self._scaled_point(angle, self.radius - 10)
            draw.line([(cx, cy), needle], fill="#202020", width=4 * self.AA_SCALE)
            draw.text(
                (cx - 18 * self.AA_SCALE, (self.center_y - 42) * self.AA_SCALE),
                f"{value_v:.3f}V",
                fill="#202020",
                font=value_font,
            )
        ...
        final = img.resize((self.width, self.height), resample)
        self._photo = ImageTk.PhotoImage(final)
        self.canvas.delete("all")
        self.canvas.create_image(0, 0, image=self._photo, anchor="nw")

    def _render_with_canvas(self, value_v: Optional[float]) -> None:
        self.canvas.delete("all")
        arc_box = (
        ...
        for start_v, end_v, color in [
            ...
            start_angle = self._angle_from_value(start_v)
            end_angle = self._angle_from_value(end_v)
            self.canvas.create_arc(arc_box, start=start_angle, extent=end_angle - start_angle, style="arc", width=9, outline=color)

        self.canvas.create_arc(arc_box, start=150 + self.ROTATION_DEG, extent=240, style="arc", width=2, outline="#4a4a4a")
        ...
        self.canvas.create_oval(self.center_x - 5, self.center_y - 5, self.center_x + 5, self.center_y + 5, fill="#303030", outline="")
        self.canvas.create_rectangle(
            self.center_x - 14,
            self.center_y - self.radius - 6,
            self.center_x + 14,
            self.center_y - self.radius + 22,
            fill="#ffffff",
            outline="#4a4a4a",
            width=1,
        )
        self.canvas.create_text(
            ...
            text=self.gauge_number,
            fill="#202020",
            font=("TkDefaultFont", 11, "bold"),
        )

        if value_v is None or value_v <= 0:
            self.canvas.create_text(self.center_x, self.center_y - 42, text="OFF", fill="#6b7280", font=("TkDefaultFont", 12, "bold"))
            self.canvas.create_line(self.center_x, self.center_y, self.center_x - self.radius + 18, self.center_y - 14, fill="#b0b6bf", width=4)
            return

        angle = self._angle_from_value(value_v)
        needle = self._point_from_angle(angle, self.radius - 10)
        self.canvas.create_line(self.center_x, self.center_y, needle[0], needle[1], fill="#202020", width=4)
        self.canvas.create_text(self.center_x, self.center_y - 42, text=f"{value_v:.3f}V", fill="#202020", font=("TkDefaultFont", 11, "bold"))
    ...
    def set_value(self, value_v: Optional[float]) -> None:
        ...
        if Image is not None and ImageDraw is not None and ImageTk is not None:
            self._render_with_pillow(value_v)
        else:
            self._render_with_canvas(value_v)
...
class App:
    PLOT_SERIES = [
        ("input_v", "Input V", "#1f77b4", "main"),
        ("battery_v", "Battery V", "#ff7f0e", "main"),
        ("current_a", "Current A", "#2ca02c", "current"),
        ("cell1", "Cell1", "#d62728", "cells"),
        ("cell2", "Cell2", "#9467bd", "cells"),
        ("cell3", "Cell3", "#8c564b", "cells"),
        ("cell4", "Cell4", "#e377c2", "cells"),
        ("cell5", "Cell5", "#7f7f7f", "cells"),
        ("cell6", "Cell6", "#17becf", "cells"),
    ]
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.plot_visibility: dict[str, tk.BooleanVar] = {
            key: tk.BooleanVar(value=True)
            for key, _, _, _ in self.PLOT_SERIES
        }
        self.plot_legend_swatches: dict[str, tk.Label] = {}
    ...
    def _build_ui(self) -> None:
        ...
        plot_controls.pack(fill="x", padx=4, pady=(0, 6))
        ttk.Label(plot_controls, text="Graph series:").pack(side="left", padx=(0, 8))
        for key, label, _, _ in self.PLOT_SERIES:
            item_frame = ttk.Frame(plot_controls)
            item_frame.pack(side="left", padx=(0, 8))
            swatch = tk.Label(
                item_frame,
                width=2,
                height=1,
                bg=self._plot_color(key),
                relief="solid",
                bd=1,
            )
            swatch.pack(side="left", padx=(0, 4))
            self.plot_legend_swatches[key] = swatch
            ttk.Checkbutton(
                item_frame,
                text=label,
                variable=self.plot_visibility[key],
                command=self.on_plot_visibility_changed,
            ).pack(side="left")
        ...
        self.ax_current.spines["left"].set_position(("outward", 58))
        self.ax_current.spines["left"].set_visible(True)
        self.ax_current.yaxis.set_label_position("left")
        self.ax_current.yaxis.set_ticks_position("left")
        ...
        self.figure.subplots_adjust(left=0.12, right=0.86, bottom=0.12, top=0.95)
    ...
    def on_plot_visibility_changed(self) -> None:
        for key, swatch in self.plot_legend_swatches.items():
            swatch.configure(bg=self._plot_color(key) if self.plot_visibility[key].get() else "#d0d0d0")
        self.plot_dirty = True

    def _plot_color(self, key: str) -> str:
        for series_key, _, color, _ in self.PLOT_SERIES:
            if series_key == key:
                return color
        return "#808080"
    ...
    def redraw_plot(self) -> None:
        ...
        self.ax_main.set_xlabel("Frame #")
        self.ax_main.set_ylabel("Input / Battery [V]", color="#303030")
        self.ax_cells.set_ylabel("Cells [V]", color="#7a3e00")
        self.ax_current.set_ylabel("Current [A]", color="#1d6f42")
        self.ax_main.tick_params(axis="y", colors="#303030")
        self.ax_cells.tick_params(axis="y", colors="#7a3e00")
        self.ax_current.tick_params(axis="y", colors="#1d6f42")
        self.ax_cells.spines["right"].set_position(("outward", 0))
        self.ax_current.spines["right"].set_visible(False)
        self.ax_current.spines["left"].set_position(("outward", 58))
        self.ax_current.spines["left"].set_visible(True)
        self.ax_current.yaxis.set_label_position("left")
        ...
        if self.history_x:
            if self.plot_visibility["input_v"].get():
                self.ax_main.plot(self.history_x, self.history_input_v, label="Input V", color="#1f77b4")

            if self.plot_visibility["battery_v"].get():
                self.ax_main.plot(self.history_x, self.history_battery_v, label="Battery V", color="#ff7f0e")
            ...
            for key, label, color in [
                ...
                if not self.plot_visibility[key].get():
                    continue
                series = self.history_cells[key]
                if any(v == v for v in series):
                    self.ax_cells.plot(self.history_x, series, label=label, color=color)

            if self.plot_visibility["current_a"].get():
                self.ax_current.plot(self.history_x, self.history_current_a, label="Current A", color="#2ca02c")
